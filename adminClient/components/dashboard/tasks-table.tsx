...
import { leads, LeadType, client, LeadSource } from "@/mock-data/dashboard-tasks";
import { useDashboardStore } from "@/store/dashboard-task-store";
...
function TypeBadge({ type }: { type: LeadType }) {
  if (type === "confirmed") {
    return (
      ...
      >
        ...
        <span className="text-sm font-medium text-cyan-400">confirmed</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-medium text-pink-400">pending</span>
    </div>
  );
}
...
function StatusBadge({ status }: { status: client }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-medium text-amber-400">no</span>
    </div>
  );
}
...
export function TasksTable() {
  ...
  const filteredAndSortedLeads = useMemo(() => {
    const result = leads.filter((lead) => {
      ...
      const matchesStatus =
        statusFilter === "all" || lead.client === statusFilter;
      const matchesSource =
    ...
    <div className="bg-card text-card-foreground rounded-xl border overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3.5 border-b">
        <div className="flex items-center gap-3">
          <h3 className="font-medium text-base">Booking Management</h3>
              ...
              <DropdownMenuContent align="start" className="w-48">
                <div className="px-2 py-1.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <DropdownMenuCheckboxItem
                      checked={typeFilter === "confirmed"}
                      onCheckedChange={() => setTypeFilter("confirmed")}
                    >
                      <Snowflake className="size-3 mr-1.5 text-cyan-400" />
                      Confirmed
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem
                      checked={typeFilter === "pending"}
                      onCheckedChange={() => setTypeFilter("pending")}
                    >
                ...
                <div className="px-2 py-1.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <DropdownMenuCheckboxItem
                      checked={statusFilter === "yes"}
                      onCheckedChange={() => setStatusFilter("yes")}
                    >
                      <Check className="size-3 mr-1.5 text-emerald-400" />
                      Yes
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem
                      checked={statusFilter === "no"}
                      onCheckedChange={() => setStatusFilter("no")}
                    >
          ...
          <TableBody>
            {paginatedLeads.map((lead) => (
              <TableRow key={lead.id} className="border-border/50">
                ...
                <TableCell>
                  <StatusBadge status={lead.client} />
                </TableCell>
                <TableCell>
